...
def MultiBoxPrior(feature_map, sizes=[0.75, 0.5, 0.25], ratios=[1, 2, 0.5]):
    ...
    """
    pairs = []  # pair of (size, sqrt(ration))  n + m - 1个组合
    for r in ratios:
    ...
    # print(ss1,ss2)
    base_anchors = np.stack([-ss1, -ss2, ss1, ss2], axis=1) / 2 #基础锚点  stack按行堆叠
    # print(base_anchors)

    h, w = feature_map.shape[-2:]
    shifts_x = np.arange(0, w) / w
    shifts_y = np.arange(0, h) / h
    shift_x, shift_y = np.meshgrid(shifts_x, shifts_y)#网格点坐标矩阵
...
X = torch.Tensor(1, 3, h, w)  # 构造输入数据  四维
# print(X)
Y = MultiBoxPrior(X, sizes=[0.75, 0.5, 0.25], ratios=[1, 2, 0.5])
print(Y.shape)  # torch.Size([1, 2042040, 4])

boxes = Y.reshape((h, w, 5, 4))
print(boxes[200, 200, 0, :])# * torch.tensor([w, h, w, h], dtype=torch.float32)
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if labels and len(labels) > i:
            ...
            axes.text(rect.xy[0], rect.xy[1], labels[i],va='center', ha='center',
                    fontsize=6, color=text_color, bbox=dict(facecolor=color, lw=0))
