...
def decode(loc, priors, variances):
    ...
    boxes = torch.cat((
        ...
        priors[:, 2:] * torch.exp(loc[:, 2:] * variances[1])), 1) #按列拼，行数不变，列数增加
    boxes[:, :2] -= boxes[:, 2:] / 2
    boxes[:, 2:] += boxes[:, :2]
    return boxes
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    v, idx = scores.sort(0)  # sort in ascending order升序
    # I = I[v >= 0.01]
    idx = idx[-top_k:]  # indices of the top-k largest valves 索引
    xx1 = boxes.new()
    yy1 = boxes.new()
    xx2 = boxes.new()
    yy2 = boxes.new()
